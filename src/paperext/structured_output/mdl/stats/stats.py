...
#         ]:
#             assert (
#                 str_normalize(domain.name.value) in _DOMAINS_CATEGORY_MAP
#             ), f"{domain.name.value}:{str_normalize(domain.name.value)} not in _DOMAINS_CATEGORY_MAP"
...
def load_single_analysis(paper, folder: Path, selector: Callable):
    paper_path = next(iter(Paper(paper).queries), None)
